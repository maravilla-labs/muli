...
export function saveBootstrapState(state) {
    ensureDirs();
...
export async function resolveAvailablePort(preferred) {
    return resolveAvailablePortWithProbe(preferred, isPortAvailable);
}
export async function resolveAvailablePortWithProbe(preferred, probe) {
...
export function buildLocalFullArgs(ports, embeddedAgent, allowLocalhostWebhooks) {
    const args = [
        '--registry',
        'full',
        '--git',
...
export function mergeDockerDaemonJson(raw, registryAddr) {
...
export function mergeNpmrc(raw, registryHost, registryPort, token) {
    ...
    const out = lines.filter(line => !line.startsWith('registry=') && !line.startsWith(`//${registryHost}:${registryPort}/-/npm/:_authToken=`));
    out.push(registryLine, tokenLine);
    return `${out.join('\n')}\n`;
}
export function mergeCargoConfig(raw, registryHost, registryPort, token) {
    const block = [
        '[registries.muli]',
...
export function mergeMavenSettings(raw, token) {
    const serverSnippet = [
        '  <server>',
        '    <id>muli</id>',
...
async function checkLocalhostTenantResolution() {
    try {
...
async function promptSecurityMode(defaultMode) {
...
function updateCliConfigFromState(config, state, apiKey) {
...
async function createBootstrapTokens(config) {
    const clients = buildClients(config);
    const registryRes = await callRpc(clients.registry, 'CreateRegistryToken', {
        tenant_id: config.tenantId,
        description: 'bootstrap-cli-local',
        ttl_seconds: 0,
        permissions: ['REGISTRY_PERMISSION_PULL', 'REGISTRY_PERMISSION_PUSH'],
    }, clients.meta);
    const gitRes = await callRpc(clients.git, 'CreateAccessToken', {
        tenant_id: config.tenantId,
        description: 'bootstrap-cli-local',
...
async function waitForGrpc(config, attempts = 40) {
    const clients = buildClients(config);
    let lastErr = null;
...
function previewWrite(filePath, content) {
...
async function maybeWrite(filePath, next, label) {
...
async function runOnboardingWriters(config, registryToken, gitToken) {
...
export async function runPostStartOnboarding(state) {
    if (!process.stdin.isTTY || !process.stdout.isTTY) {
        return;
    }
...
export async function prepareServerStart(options) {
    const state = loadBootstrapState();
    ...
    const args = buildLocalFullArgs({ registryPort: state.registryPort, gitPort: state.gitPort }, embeddedAgent, state.allowLocalhostWebhooks);
    const env = {
