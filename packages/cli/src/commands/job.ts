...
export function registerJobCommands(program: Command): void {
  ...
  job
    ...
    .action(async (cmdArgs: string[], opts, cmd) => {
      const config = loadConfig();
      const clients = buildClients(config);

      // Parse env vars
      const envVars = (opts.env as string[]).map((kv: string) => {
      ...
      // Build resource spec
      const resources: any = {};
      if (opts.cpu) resources.cpu_millis = parseCpu(opts.cpu);
      if (opts.mem) resources.memory_bytes = parseMemory(opts.mem);
      ...
      try {
        ...
        console.log(
          chalk.green('✓'),
          `Job submitted: ${chalk.bold(jobId)}`,
        );
        console.log();
        ...
        const { waitUntilExit } = render(
          React.createElement(LogStream, {
            jobId,
            clients,
            follow: true,
            onDone: (code: number) => { exitCode = code; },
          }),
        );
        await waitUntilExit();
      ...
      if (opts.follow) {
        ...
        try {
          ...
          for (const entry of entries) {
            const streamLabel =
              String(entry.stream ?? '').toUpperCase() === 'STDERR'
                ? chalk.red('[stderr]')
                : chalk.dim('[stdout]');
...
function colorState(state?: string): string {
  if (!state) return chalk.dim('unknown');
