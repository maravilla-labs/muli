...
export function registerAuthCommands(program: Command): void {
  ...
  auth
    ...
    .action(async (serverUrl?: string) => {
      ...
      try {
        ...
        console.error(
          chalk.red(`Cannot reach server at ${url}:`),
          err.message,
        );
        process.exit(1);
      }
      ...
      console.log(
        chalk.green('✓'),
        `Connected to ${chalk.bold(url)}, tenant: ${chalk.bold(newConfig.tenantId)}`,
      );
    });
  ...
  auth
    ...
    .action(() => {
      ...
      console.log(
        `API key:  ${config.apiKey ? chalk.green('set') : chalk.dim('not set (dev mode)')}`,
      );
