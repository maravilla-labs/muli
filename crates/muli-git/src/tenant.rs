...
//! Tenant configuration and routing for git repositories.
...
pub struct TenantConfig {
    /// Base domain for tenant subdomains (e.g., "git.example.com").
...
/// Paths that are allowed without tenant context.
...
pub async fn tenant_middleware(request: Request, next: Next) -> Response {
    ...
    if !validate_tenant_slug(&slug) {
        return json_error(StatusCode::BAD_REQUEST, "invalid tenant slug");
    }
...
mod tests {
    ...
    fn extract_valid_tenant_slugs() {
        assert_eq!(
            extract_tenant_slug("tenant-123.git.example.com", "git.example.com"),
            Some("tenant-123")
        );
        assert_eq!(
            extract_tenant_slug("myorg.git.example.com", "git.example.com"),
            Some("myorg")
        );
        assert_eq!(
            extract_tenant_slug("abc.git.example.com:3000", "git.example.com"),
            Some("abc")
        );
    ...
    fn extract_no_subdomain() {
        assert_eq!(
            extract_tenant_slug("git.example.com", "git.example.com"),
            None
        );
    }
    ...
    fn extract_wrong_domain() {
        assert_eq!(
            extract_tenant_slug("tenant.other.com", "git.example.com"),
            None
        );
    }
    ...
    fn extract_nested_subdomain_rejected() {
        assert_eq!(
            extract_tenant_slug("a.b.git.example.com", "git.example.com"),
            None
        );
    }
